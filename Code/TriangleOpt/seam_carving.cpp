...
static float sobel_filter_at(const MatrixF &m, int cx, int cy) {
    ...
    for(int dx=-1; dx<=1; ++dx) {
      int x=cx+dx, y=cy+dy;
      float v = (y>=0&&y<m.height&&x>=0&&x<m.width) 
                ? m.at(y,x) : 0.f;
      sx += v*gx[dy+1][dx+1];
      sy += v*gy[dy+1][dx+1];
    }
...
static void grad_to_dp_animated(const MatrixF &grad,
                                MatrixF &dp,
                                int base_w)
{
    int W = grad.width, H = grad.height;

    // 1) Compute DP *and record* every (y,x) we write to.
    ...
    if(base_w >= W){
        for(int y=1; y<H; ++y){
            ...
            for(int cx=0; cx<W; ++cx){
                ...
                #pragma omp critical
                visits.emplace_back(y,cx);
            }
        }
    } else {
        int tri_h      = base_w/3 + 1;
    ...
    std::cout << "DP computation done! Recorded " 
              << visits.size() << " visits.\n";

    // 2) Playback
    cv::Mat anim(H, W, CV_8UC3, cv::Scalar(255,255,255));
    cv::VideoWriter writer("dp_anim.avi",
        cv::VideoWriter::fourcc('M','J','P','G'),
        60, cv::Size(W,H));
    cv::namedWindow("DP Animation", cv::WINDOW_NORMAL);
    cv::resizeWindow("DP Animation", W*2, H*2);

    for(auto [y,x] : visits){
        anim.at<cv::Vec3b>(y,x) = {0,0,255};
        cv::imshow("DP Animation", anim);
        writer.write(anim);
        cv::waitKey(1);
    }

    std::cout<<"Animation done!  Press any key to exit.\n";
    cv::waitKey(0);
    writer.release();
    cv::destroyWindow("DP Animation");
}
...
static void compute_seam(const MatrixF &dp, std::vector<int> &seam){
    ...
    seam.resize(H);
    seam[H-1] = std::min_element(&dp.at(H-1,0), &dp.at(H-1,0)+W)
                - &dp.at(H-1,0);
    for(int y=H-2; y>=0; --y){
        int px=seam[y+1], best_i=px;
...
static void markout_sobel_patches(MatrixF &g,
...
static void print_usage(const char *p){
    std::cout<<"Usage: "<<p
             <<" [-proc <threads>] [-width <base_w>] <in.png> <out.png>\n";
}
...
int main(int argc,char*argv[]){
    ...
    if(argc-argi>1 && std::string(argv[argi])=="-width"){
    ...
    if(input.empty()){ std::cerr<<"ERROR loading "<<inF<<"\n"; return 1; }
    cv::Mat bgra;
    if(input.channels()==3)      cv::cvtColor(input,bgra,cv::COLOR_BGR2BGRA);
    else if(input.channels()==4) bgra=input.clone();
    ...
    for(int i=0; i<seams_to_remove; ++i){
        grad_to_dp_animated(grad, dp, base_w);
